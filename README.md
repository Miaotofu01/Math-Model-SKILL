...
- **多子问题逐问处理**：多小问的题自动拆解，每问独立建模、求解、写作，对齐逐问评分
- **建模与求解闭环**：建模方案过专家评审（最多 6 轮改稿）；求解代码真实运行并多方式验证，不达标继续迭代，直到收敛
- **摘要数字溯源 + 代码对账**：摘要每个数字在正文有出处，且与代码真实运行结果逐一对账
- **论文模板内置**：国赛格式固化在模板里，直接产出合规 PDF（摘要无编号、2.5cm 页边距、附录 A/B/C、支撑材料清单 + AI 使用声明，换赛题无需改模板；美赛走独立英文路径）
