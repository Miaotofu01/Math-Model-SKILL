...
> **v2.3.0** · 基于 Claude Code 的 Agent 编排插件 · 国赛 CUMCM / 美赛 MCM
...
| 7. 写作 | 事实源表 → 叙事大纲 → 顺序主编撰写 → 交叉审查统一修复（max 3 轮） | 质量门：章节 / 排序 / 溯源 / 图表 / P0；全篇数字以事实源表为唯一权威 |
...
- **事实源表**：写作前从最终求解结果提取全篇唯一数字权威（关键数字 / 符号 / 口径 / 数据画像），后续所有章节与修复只准引用这张表，杜绝不同章节数字版本不一致。
...
└── intermediates/  # 03-search / 04-proposal / 05-writing / 06-final / 07-fact-sheet / ...
```
